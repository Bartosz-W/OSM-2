...
// returns resulting length or < 0 on failure
    int DWHCIDeviceGetDescriptor(TDWHCIDevice *pThis, TUSBEndpoint *pEndpoint,
	    unsigned char ucType, unsigned char ucIndex, void *pBuffer,
	    unsigned nBufSize, unsigned char ucRequestType /* = REQUEST_IN */);
    ...
    boolean DWHCIDeviceSetConfiguration(TDWHCIDevice *pThis,
	    TUSBEndpoint *pEndpoint, u8 ucConfigurationValue);
