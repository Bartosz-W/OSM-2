...
// This handler is called when Ctrl-Alt-Del is pressed.
    ...
    void USPiKeyboardRegisterShutdownHandler(
	    TUSPiShutdownHandler *pShutdownHandler);
...
// The status handler is called when the mouse sends a status report.
    typedef void TUSPiMouseStatusHandler(unsigned nButtons, int nDisplacementX,	// -127..127
	    int nDisplacementY);		// -127..127
    void USPiMouseRegisterStatusHandler(
	    TUSPiMouseStatusHandler *pStatusHandler);
...
// returns 0 on failure
    ...
    void USPiGamePadRegisterStatusHandler(
